...
<div class="mx-auto max-w-7xl px-4 py-8 md:px-8" style="font-family: 'DM Sans', sans-serif;">
	...
	{:else}
		<!-- Services Grid -->
		<div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3 transition-all duration-500 {visible ? 'translate-y-0 opacity-100' : 'translate-y-3 opacity-0'}">
			{#each services as service, i}
				<div
					class="rounded-2xl border {isDark ? 'border-white/[0.04]' : 'border-zinc-200'} {isDark ? 'bg-white/[0.02]' : 'bg-white'} p-6 transition-all {isDark ? 'hover:border-white/[0.08]' : 'hover:border-zinc-300'}"
						...
						<div class="flex items-start justify-between">
							...
							<div class="ml-3 flex items-center gap-1">
								<button
									...
									class="rounded-lg p-1.5 {isDark ? 'text-white/20 hover:text-white/50 hover:bg-white/[0.04]' : 'text-zinc-400 hover:text-zinc-600 hover:bg-zinc-50'} transition-all"
									title="Edit"
								>
								...
								{#if confirmDeleteId === service.id}
									<button
										...
										class="rounded-lg px-2 py-1 text-[11px] font-medium text-red-500 bg-red-500/10 hover:bg-red-500/20 transition-all disabled:opacity-50"
									>
										{deletingId === service.id ? '...' : 'Confirm'}
									</button>
									<button
										onclick={() => { confirmDeleteId = null; }}
										class="rounded-lg p-1.5 {isDark ? 'text-white/20 hover:text-white/50' : 'text-zinc-400 hover:text-zinc-600'} transition-all"
									>
										<svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
								...
								{:else}
									<button
										onclick={() => { confirmDeleteId = service.id; }}
										class="rounded-lg p-1.5 {isDark ? 'text-white/20 hover:text-red-400 hover:bg-red-500/10' : 'text-zinc-400 hover:text-red-500 hover:bg-red-50'} transition-all"
										title="Delete"
									>
										<svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
