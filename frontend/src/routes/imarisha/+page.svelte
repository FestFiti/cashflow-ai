...
<div class="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950" style="font-family: 'DM Sans', sans-serif;">
	{#if step === 'login'}
		<!-- Splash / Login -->
		<div class="flex min-h-screen items-center justify-center px-4">
			<div class="w-full max-w-md transition-all {visible ? 'translate-y-0 opacity-100' : 'translate-y-3 opacity-0'}">
				...
				<div class="mb-8 text-center">
					...
					<h1 class="font-['Instrument_Serif'] text-2xl tracking-tight text-white">Imarisha</h1>
					<p class="mt-2 text-sm text-white/60">Automated Chama Management, Fully Trustworthy</p>
				</div>

				<!-- Phone Login -->
				<div class="rounded-2xl border border-white/[0.04] bg-white/[0.02] p-6">
					<div class="space-y-4">
						<div>
							<label class="mb-2 block text-sm font-medium text-white/80">Phone Number</label>
							<input
								type="tel"
								bind:value={phone}
						...
						{#if otp}
							<div>
								<label class="mb-2 block text-sm font-medium text-white/80">Enter OTP</label>
								<input
									type="text"
									bind:value={otp}
									placeholder="123456"
				...
				{#if !otp}
					<div class="mt-6 text-center">
						<button class="rounded-xl border border-white/[0.06] bg-white/[0.02] px-8 py-3 text-sm font-medium text-white transition-colors hover:bg-white/[0.05]">
							Get Started
						</button>
					</div>
				{/if}
		...
		<div class="flex min-h-screen items-center justify-center px-4">
			<div class="w-full max-w-md transition-all {visible ? 'translate-y-0 opacity-100' : 'translate-y-3 opacity-0'}">
				<div class="mb-6 text-center">
					<h2 class="font-['Instrument_Serif'] text-xl tracking-tight text-white">Setup Your Profile</h2>
				</div>
				...
				<div class="rounded-2xl border border-white/[0.04] bg-white/[0.02] p-6">
					<div class="space-y-4">
						...
						<div>
							<label class="mb-2 block text-sm font-medium text-white/80">Photo (Optional)</label>
							<div class="flex items-center gap-4">
								{#if photoUrl}
									<img src={photoUrl} alt="Profile" class="h-16 w-16 rounded-full object-cover" />
								{:else}
									<div class="flex h-16 w-16 items-center justify-center rounded-full bg-white/[0.05]">
								...
								{/if}
								<button class="rounded-lg border border-white/[0.06] bg-white/[0.02] px-3 py-2 text-xs font-medium text-white/60 transition-colors hover:bg-white/[0.05]">
						...
						<div>
							<label class="mb-2 block text-sm font-medium text-white/80">ID Number (Optional)</label>
							<input
								type="text"
								bind:value={idNumber}
						...
						<div>
							<label class="mb-2 block text-sm font-medium text-white/80">Preferred Language</label>
							<select
								bind:value={language}
								class="w-full rounded-xl border border-white/[0.06] bg-white/[0.03] px-4 py-3 text-white outline-none transition-colors focus:border-emerald-500/50 focus:bg-white/[0.05]"
							>
	...
	{:else if step === 'join'}
		...
		<div class="flex min-h-screen items-center justify-center px-4">
			<div class="w-full max-w-md transition-all {visible ? 'translate-y-0 opacity-100' : 'translate-y-3 opacity-0'}">
				<div class="mb-6 text-center">
					<h2 class="font-['Instrument_Serif'] text-xl tracking-tight text-white">Join or Create a Chama</h2>
				</div>

				<!-- Action Toggle -->
				<div class="mb-6 flex gap-2 rounded-xl border border-white/[0.04] bg-white/[0.02] p-1">
					<button
						onclick={() => (groupAction = 'create')}
						class="flex-1 rounded-lg px-4 py-2 text-sm font-medium transition-all {groupAction === 'create' ? 'bg-emerald-500 text-zinc-950' : 'text-white/60'}"
					>
						Create Group
					</button>
					<button
						onclick={() => (groupAction = 'join')}
						class="flex-1 rounded-lg px-4 py-2 text-sm font-medium transition-all {groupAction === 'join' ? 'bg-emerald-500 text-zinc-950' : 'text-white/60'}"
					>
						Join Group
					</button>
				</div>
				...
				<div class="rounded-2xl border border-white/[0.04] bg-white/[0.02] p-6">
					{#if groupAction === 'create'}
						...
						<div class="space-y-4">
							<div>
								<label class="mb-2 block text-sm font-medium text-white/80">Group Name</label>
								<input
									type="text"
							...
							<div>
								<label class="mb-2 block text-sm font-medium text-white/80">Contribution Amount (KES)</label>
								<input
									type="number"
							...
							<div>
								<label class="mb-2 block text-sm font-medium text-white/80">Frequency</label>
								<select
					...
					{:else}
						...
						<div class="space-y-4">
							<div>
								<label class="mb-2 block text-sm font-medium text-white/80">Group Code / QR Scan</label>
								<input
									type="text"
