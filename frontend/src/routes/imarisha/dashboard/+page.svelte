...
<div class="mx-auto max-w-7xl px-4 py-8 md:px-8" style="font-family: 'DM Sans', sans-serif;">
	...
	<div class="mb-8 flex items-end justify-between">
		<div>
			<p class="mb-1 text-[12px] font-medium uppercase tracking-[0.15em] text-white/25">Chama Dashboard</p>
	...
	{:else if data}
		...
		<div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
			...
			<div class="rounded-2xl border border-emerald-500/10 bg-emerald-500/[0.03] p-6 transition-all duration-500 {visible ? 'translate-y-0 opacity-100' : 'translate-y-3 opacity-0'}">
				...
				<p class="text-2xl font-bold tracking-tight text-white">{formatKES(data.expected_inflow)}</p>
				<p class="mt-2 text-[11px] text-white/20">scheduled for automatic deduction</p>
			</div>
			...
			<div class="rounded-2xl border border-amber-500/10 bg-amber-500/[0.03] p-6 transition-all duration-500 delay-75 {visible ? 'translate-y-0 opacity-100' : 'translate-y-3 opacity-0'}">
				...
				<p class="text-2xl font-bold tracking-tight text-white">{formatKES(data.money_in_motion)}</p>
				<p class="mt-2 text-[11px] text-white/20">pending STK pushes</p>
			</div>
			...
			<div class="rounded-2xl border border-white/[0.04] bg-white/[0.02] p-6 transition-all duration-500 delay-150 {visible ? 'translate-y-0 opacity-100' : 'translate-y-3 opacity-0'}">
				...
				<p class="text-2xl font-bold tracking-tight text-white">{formatKES(data.collected)}</p>
				<p class="mt-2 text-[11px] text-white/20">completed contributions</p>
			</div>
			...
			<div class="rounded-2xl border border-red-500/10 bg-red-500/[0.03] p-6 transition-all duration-500 delay-200 {visible ? 'translate-y-0 opacity-100' : 'translate-y-3 opacity-0'}">
				...
				<p class="text-2xl font-bold tracking-tight text-white">{formatKES(data.at_risk)}</p>
				<p class="mt-2 text-[11px] text-white/20">failed or delayed transactions</p>
			</div>
		</div>
		...
		<div class="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-12">
			...
			<div class="rounded-2xl border border-white/[0.04] bg-white/[0.02] p-6 lg:col-span-8 transition-all duration-500 delay-300 {visible ? 'translate-y-0 opacity-100' : 'translate-y-3 opacity-0'}">
				<div class="mb-6 flex items-center justify-between">
					<span class="text-[11px] font-medium uppercase tracking-[0.12em] text-white/25">Contributions Feed</span>
							...
							<div class="flex items-center justify-between">
								<div class="flex items-center gap-3">
									<div class="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-500/10">
										<span class="text-[10px] font-bold text-emerald-400">{contribution.member.charAt(0).toUpperCase()}</span>
									</div>
									<div>
										<p class="text-[13px] font-medium text-white/80">{contribution.member}</p>
										<p class="text-[11px] text-white/40">{contribution.rule} • {contribution.timestamp}</p>
									</div>
								</div>
								<div class="text-right">
									<p class="text-lg font-medium text-white">{formatKES(contribution.amount)}</p>
			...
			<div class="space-y-6 lg:col-span-4">
				<!-- Quick Actions -->
				<div class="rounded-2xl border border-white/[0.04] bg-white/[0.02] p-6 transition-all duration-500 delay-400 {visible ? 'translate-y-0 opacity-100' : 'translate-y-3 opacity-0'}">
					<span class="mb-4 block text-[11px] font-medium uppercase tracking-[0.12em] text-white/25">Quick Actions</span>
					<div class="space-y-2">
						<a href="/imarisha/contributions" class="flex items-center gap-3 rounded-xl border border-white/[0.04] p-3 transition-all hover:border-white/[0.08] hover:bg-white/[0.02]">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-white/30" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
								<path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
							</svg>
							<div>
								<p class="text-[13px] font-medium text-white/80">View Contribution Details</p>
								<p class="text-[11px] text-white/20">Full audit trail</p>
							</div>
						</a>
						<button class="flex items-center gap-3 rounded-xl border border-white/[0.04] p-3 transition-all hover:border-white/[0.08] hover:bg-white/[0.02]">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-white/30" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
							...
							<div>
								<p class="text-[13px] font-medium text-white/80">Send Reminder</p>
								<p class="text-[11px] text-white/20">For failed deductions</p>
							</div>
						</button>
						<button class="flex items-center gap-3 rounded-xl border border-white/[0.04] p-3 transition-all hover:border-white/[0.08] hover:bg-white/[0.02]">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-white/30" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
							...
							<div>
								<p class="text-[13px] font-medium text-white/80">Add Member / Edit Rules</p>
								<p class="text-[11px] text-white/20">Manage group settings</p>
							</div>
						</button>
					</div>
				</div>
				...
				<div class="rounded-2xl border border-emerald-500/10 bg-emerald-500/[0.03] p-6 transition-all duration-500 delay-500 {visible ? 'translate-y-0 opacity-100' : 'translate-y-3 opacity-0'}">
					...
					<p class="text-[12px] text-white/60">All contributions are automatically processed and tracked.</p>
					<div class="mt-3">
						<div class="mb-2 flex items-center justify-between text-[10px] text-white/40">
