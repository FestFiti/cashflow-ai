		...
		<div class="rounded-2xl border {isDark ? 'border-white/[0.04]' : 'border-zinc-200'} {isDark ? 'bg-white/[0.02]' : 'bg-white'} p-6 md:p-8 transition-all duration-500 {visible ? 'translate-y-0 opacity-100' : 'translate-y-3 opacity-0'}">
					...
					<div>
						...
						<div class="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1">
							...
							{#if invoice.client_email}
								<span class="text-[12px] {isDark ? 'text-white/20' : 'text-zinc-400'}">{invoice.client_email}</span>
							{/if}
						</div>
					</div>
				</div>
			...
			{#if invoice.status !== 'paid'}
				<div class="mt-8 flex gap-3">
					...
					<button
						class="flex items-center gap-2 rounded-xl border {isDark ? 'border-white/[0.08]' : 'border-zinc-200'} px-5 py-2.5 text-[13px] font-medium {isDark ? 'text-white/40' : 'text-zinc-500'} transition-all {isDark ? 'hover:border-white/[0.15]' : 'hover:border-zinc-300'} {isDark ? 'hover:text-white/60' : 'hover:text-zinc-700'}"
					>
